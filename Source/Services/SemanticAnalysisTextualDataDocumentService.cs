...
namespace SemanticaAnalysisTextualData.Source.Services
{
    ...
    /// </summary>
    public class SemanticAnalysisTextualDataDocumentService : ISemanticAnalysisTextualDataInterface
    {
        ...
        public async Task<double> CalculateSimilarityAsync()
        {
            ...
            var documents = new List<string>
             {
          File.ReadAllText("C:\\Users\\ASUS\\source\\repos\\Tech_Tweakers\\SemanticaAnalysisTextualData\\SemanticaAnalysisTextualData\\data\\AngeloMarker.txt"),
    File.ReadAllText("C:\\Users\\ASUS\\source\\repos\\Tech_Tweakers\\SemanticaAnalysisTextualData\\SemanticaAnalysisTextualData\\data\\ChristianoRonaldo.txt"),
};
            ...
            var embeddings = await Task.WhenAll(documents.Select(async doc =>
            {
                try
                {
                    Console.WriteLine($"Generating embedding for document: {doc.Substring(0, Math.Min(50, doc.Length))}...");
                    var embedding = await client.GenerateEmbeddingAsync(doc);
            ...
            if (embeddings.Any(e => e == null))
            {
                Console.WriteLine("Some embeddings failed to generate.");
            }
            return 1;
            //}
        }
